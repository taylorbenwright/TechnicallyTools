...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        if self.settings["autoBend"]:
            ...
            self.spine_ik_cns = primitive.addTransform(
                self.ik1autoRot_lvl, self.getName('ik1_ik_cns'), t)

            self.ik1_ctl_zero = primitive.addTransform(
                self.spine_ik_cns, self.getName('ik1_ctl_zero'), t)
            ...
            self.spine_ik_cns = primitive.addTransform(
                self.ik1_npo, self.getName('ik1_ik_cns'), t)
    ...
    def addAttributes(self):
        ...
        self.spine_ikref_att = None
        if self.settings['spine_ikref_array']:
            spine_ref_names = self.get_valid_alias_list(self.settings["spine_ikref_array"].split(","))
            if len(spine_ref_names) > 1:
                self.spine_ikref_att = self.addAnimEnumParam("spineikref", "Grip Ik Ref", 0, spine_ref_names)
    ...
    def connect_standard(self):
        ...
        if self.spine_ikref_att is not None:
            self.connectRef2(self.settings['spine_ikref_array'], self.spine_ik_cns, self.spine_ikref_att)
